...
async def check_cut_audio(
    ...
    """
    logger.info(
        f"Testing cut_audio_into_pieces with {'memory profiler' if use_memory_profiler else 'standard'} implementation"
    )

    # Create a specific output directory for this test
    test_output_dir = (
        output_dir
        / f"{audio_path.stem}_{'profiled' if use_memory_profiler else 'standard'}"
    )
    ...
    expected_chunks = max(
        1, int((original_duration - overlap_duration) / effective_chunk_duration) + 1
    )
    ...
    try:
        ...
        if len(audio_pieces) != expected_chunks:
            logger.warning(
                f"Number of pieces ({len(audio_pieces)}) doesn't match expected ({expected_chunks})"
            )
        ...
        for piece in audio_pieces:
            ...
            try:
                ...
                if duration > chunk_duration + 1:  # Allow 1 second tolerance
                    logger.warning(
                        f"Piece {piece.name} duration ({duration:.2f}s) exceeds chunk duration ({chunk_duration}s)"
                    )
...
async def main():
    ...
    for audio_file in audio_files:
        ...
        success_standard = await check_cut_audio(
            audio_path=audio_file, output_dir=output_dir, use_memory_profiler=False
        )

        # Test memory-profiled implementation
        success_profiled = await check_cut_audio(
            audio_path=audio_file, output_dir=output_dir, use_memory_profiler=True
        )

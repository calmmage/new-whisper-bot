...
async def check_parse_audio_chunk(
    ...
    try:
        ...
        if len(transcription) == 0:
            logger.error(f"Transcription is empty")
            return False
...
async def check_parse_audio_chunks(
    ...
    try:
        ...
        if len(transcriptions) != len(audio_paths):
            logger.error(f"Number of transcriptions ({len(transcriptions)}) doesn't match number of audio files ({len(audio_paths)})")
            return False
        ...
        for i, transcription in enumerate(transcriptions):
            if len(transcription) == 0:
                ...
                return False
            logger.info(f"Transcription {i+1} length: {len(transcription)} characters")

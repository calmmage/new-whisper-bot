...
async def check_convert_video(
    ...
    """
    logger.info(f"Testing convert_video_to_audio with {'memory profiler' if use_memory_profiler else 'standard'} implementation")
    
    # Create the output directory if it doesn't exist
    output_dir.mkdir(parents=True, exist_ok=True)
    
    # Set the output path
    output_path = output_dir / f"{video_path.stem}_{'profiled' if use_memory_profiler else 'standard'}.mp3"
    
    # Convert the video to audio
    try:
        start_time = asyncio.get_event_loop().time()
        audio_path = await convert_video_to_audio(
...
async def main():
    ...
    video_files = [f for f in sample_dir.glob("*.mp4")]
    video_files += [
